	...
	return (
		<div
			...
			className={cn(
				...
				variant === 'icon' &&
					"flex size-12 shrink-0 items-center justify-center rounded-lg bg-muted text-foreground [&_svg:not([class*='size-'])]:size-6",
				className,
			)}
			{...props}
		/>
...
function EmptyDescription({ className, ...props }: React.ComponentProps<'p'>) {
	return (
		<p
			...
			className={cn(
				'text-sm/6 text-muted-foreground [&>a]:underline-offset-4 [&>a:hover]:text-primary [&>a]:hover:underline',
				className,
			)}
			{...props}
		/>
